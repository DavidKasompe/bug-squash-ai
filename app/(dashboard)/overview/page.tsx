...
import { RepoSelector, type SelectorRepo } from "@/components/mirai/repo-selector";
...
function UserBubble({ content }: { content: string }) {
  return (
    <div className="flex justify-end">
      <div className="max-w-[85%] bg-[#2A6948] text-white rounded-2xl rounded-br-sm px-5 py-3.5 shadow-sm shadow-[#2A6948]/20">
...
export default function OverviewPage() {
  ...
  const [repositories, setRepositories] = useState<SelectorRepo[]>([]);
    ...
    async function loadRepositories() {
      try {
        ...
        const nextRepositories = ((payload.installations as Array<{
          ...
          source?: string;
          repos?: Array<{ full_name: string; language?: string | null; private?: boolean }>;
          ...
          (installation.repos ?? []).map((repo) => ({
            ...
            source: (installation.source === "oauth" ? "oauth" : "app") as "oauth" | "app",
          })),
        );
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {/* Top bar */}
      <div className="flex items-center gap-3 border-b border-black/[0.05] bg-white/90 px-5 py-3 backdrop-blur shrink-0 overflow-x-auto">
        {/* Identity */}
        <div className="flex shrink-0 items-center gap-2 pr-3 border-r border-black/[0.07]">
          <Bug className="w-4 h-4 text-[#2A6948]" />
          <span className="font-bold text-sm text-[#111111] whitespace-nowrap">AI Debugging</span>
          <span className="flex items-center gap-1 text-[10px] font-bold text-[#2A6948]">
        ...
        {/* Session group */}
        <div className="flex shrink-0 items-center gap-1.5 pr-3 border-r border-black/[0.07]">
          <select
            value={sessionFilter}
            onChange={(event) => setSessionFilter(event.target.value as "all" | "bug-chat" | "commit-analysis")}
            className="rounded-lg border border-black/[0.08] bg-[#F5F4F0] px-2.5 py-1.5 text-xs font-semibold text-black/55 outline-none"
          >
            <option value="all">All</option>
            <option value="bug-chat">Bugs</option>
            <option value="commit-analysis">Commits</option>
          </select>
          <select
            value={sessionId ?? ""}
            onChange={(event) => { void handleSessionChange(event.target.value); }}
            className="w-48 rounded-lg border border-black/[0.08] bg-[#F5F4F0] px-2.5 py-1.5 text-xs font-semibold text-black/55 outline-none"
          >
        ...
        <div className="flex shrink-0 items-center gap-1.5 pr-3 border-r border-black/[0.07]">
          ...
          <input
            ...
            placeholder="Branch"
            className="w-28 rounded-lg border border-black/[0.08] bg-[#F5F4F0] px-2.5 py-1.5 text-xs font-semibold text-black/55 outline-none placeholder:text-black/25"
          />
        ...
        <div className="flex shrink-0 items-center gap-1 pr-3 border-r border-black/[0.07]">
          <button
            type="button"
            onClick={() => handleCommitQuickAction("latest")}
            disabled={!selectedRepo || isLoading}
            className="flex items-center gap-1.5 rounded-lg border border-black/[0.08] bg-white px-2.5 py-1.5 text-xs font-semibold text-black/55 hover:bg-black/[0.03] disabled:cursor-not-allowed disabled:opacity-40"
          >
            <Zap className="w-3 h-3 text-[#2A6948]" /> Last commit
          </button>
          <button
            type="button"
            ...
            className="flex items-center gap-1.5 rounded-lg border border-black/[0.08] bg-white px-2.5 py-1.5 text-xs font-semibold text-black/55 hover:bg-black/[0.03] disabled:cursor-not-allowed disabled:opacity-40"
          >
            <GitMerge className="w-3 h-3 text-[#2A6948]" /> Last 2
          </button>
          <button
            type="button"
            ...
            className="flex items-center gap-1.5 rounded-lg border border-black/[0.08] bg-white px-2.5 py-1.5 text-xs font-semibold text-black/55 hover:bg-black/[0.03] disabled:cursor-not-allowed disabled:opacity-40"
          >
            <GitMerge className="w-3 h-3 text-[#2A6948]" /> Last 3
          </button>
        </div>
        ...
        <div className="ml-auto flex shrink-0 items-center gap-1.5">
          <button
            ...
            className="flex items-center gap-1.5 rounded-lg border border-black/[0.08] bg-white px-2.5 py-1.5 text-xs font-semibold text-black/55 hover:bg-black/[0.03]"
          >
            <Plus className="w-3.5 h-3.5" /> New
          </button>
          <Link
            href="/issues"
            className="flex items-center gap-1.5 rounded-lg bg-[#2A6948] px-3 py-1.5 text-xs font-bold text-white transition-colors hover:bg-[#1E4A31]"
          >
            All issues
          </Link>
        </div>
      ...
      {/* Chat body */}
      <div className="flex-1 min-h-0 overflow-y-auto px-6 md:px-10 py-8 space-y-8">
        {messages.length === 0 && (
